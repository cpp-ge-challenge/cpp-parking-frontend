...
run:
	docker run -d --name=parking-frontend -p 80:80 -v `pwd`/static:/usr/share/nginx/html parking-frontend
...
dev:
	...
	docker run --name parking-frontend -d -p 8000:80 -v `pwd`/static:/usr/share/nginx/html -v `pwd`/nginx-dev.conf:/etc/nginx/conf.d/default.conf parking-frontend
	#run build container
